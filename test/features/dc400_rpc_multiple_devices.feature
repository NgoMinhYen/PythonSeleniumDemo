...
Feature: DC400 RPC Multiple Devices
    ...
    Scenario Outline: GSX-4298	User can set FCWS config for a device in RPC Multiple Devices
        ...
        Examples:
            | email                              | password         | deviceID         | config-set | config-get | parameters | params | value |
            | phat.ngo+tenant-admin@logigear.com | Y9!ynp7GY-XHEKWN | 0203030521054073 | config-set | config-get | {"FCWS":3} | FCWS   | 3     |